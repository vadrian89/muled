...
SOURCE_DIR=`config_property SOURCE_DIR "${CONFIG}"`
ASSETS=`config_property OTHER "${CONFIG}"`
...
QML_SOURCE="${SOURCE_DIR}/${QML_SOURCE}"
...
echo "SOURCE_DIR: ${SOURCE_DIR}" 2>&1 | tee -a "${LOG_FILE}"
echo "QML_SOURCE: ${QML_SOURCE}" 2>&1 | tee -a "${LOG_FILE}"
echo "ASSETS: ${ASSETS}" 2>&1 | tee -a "${LOG_FILE}"
...
echo "Copying extra assets" 2>&1 | tee -a "${LOG_FILE}"
IFS=" "
read -ra ASSETS_ARRAY <<< "${ASSETS}"
for asset in "${ASSETS_ARRAY[@]}"; do
  echo "Copying ${SOURCE_DIR}/${asset} to ${WORKING_DIR}/${asset}"
  if [ -d "${SOURCE_DIR}/${asset}" ]; then
    cp -r "${SOURCE_DIR}/${asset}" "${WORKING_DIR}/" 2>&1 | tee -a "${LOG_FILE}"
  else
    if [ -f "${SOURCE_DIR}/${asset}" ]; then
      cp "${SOURCE_DIR}/${asset}" "${WORKING_DIR}/${asset}" 2>&1 | tee -a "${LOG_FILE}"
    else
      echo "ERROR copying ${SOURCE_DIR}/${asset}, it is neither a file nor a directory and wasn't copied" 2>&1 | tee -a "${LOG_FILE}"
      exit 1
    fi
  fi
done
...
echo "export LD_LIBRARY_PATH=\"\${dirname}/lib\"" >> "${EXECUTABLE}.sh"
